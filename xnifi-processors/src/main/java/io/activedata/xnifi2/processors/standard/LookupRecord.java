...
package io.activedata.xnifi2.processors.standard;
...
@DynamicProperty(name = "用于Lookup的值", value = "有效的RecordPath", expressionLanguageScope = ExpressionLanguageScope.FLOWFILE_ATTRIBUTES,
...
public class LookupRecord extends AbstractRouteRecord<Tuple<Map<String, RecordPath>, RecordPath>> {
    ...
    static final AllowableValue ROUTE_TO_SUCCESS = new AllowableValue("route-to-success", "全部路由到成功队列",
        "不管检索结果是否存在，记录都会路由到成功队列中。");
    static final AllowableValue ROUTE_TO_MATCHED_UNMATCHED = new AllowableValue("route-to-matched-unmatched", "路由到匹配或未匹配队列",
        "当检索结果存在时则路由到匹配队列，否则路由到未匹配队列。");

    static final AllowableValue RESULT_ENTIRE_RECORD = new AllowableValue("insert-entire-record", "整条记录",
        "整条结果记录将会被放到指定字段上。.");
    ...
    static final PropertyDescriptor ROUTING_STRATEGY = new PropertyDescriptor.Builder()
        ...
        .displayName("路由策略")
        .description("在执行Lookup服务后路的路由策略。")
    ...
    protected PropertyDescriptor getSupportedDynamicPropertyDescriptor(final String propertyDescriptorName) {
        return new PropertyDescriptor.Builder()
            ...
            .description("存在检索结果的字段（RecordPath）")
            .addValidator(new RecordPathValidator())
            .expressionLanguageSupported(ExpressionLanguageScope.FLOWFILE_ATTRIBUTES)
            .required(false)
            .dynamic(true)

 ...
 *
 */
public class AsyncInputStreamCallback implements InputStreamCallback {
    private static final CompletableFuture[] COMPLETABLE_FUTURES = new CompletableFuture[]{};
    ...
    public AsyncInputStreamCallback(AbstractBatchProcessor processor, RecordReaderFactory readerFactory, RecordSetWriterFactory writerFactory, ComponentLog logger, FlowFile original, ProcessContext context, ProcessSession session, String writerSchemaStrategy) {
        ...
        try (final RecordReader reader = readerFactory.createRecordReader(attributes, in, -1, logger)) {
            ...
            while ((record = reader.nextRecord()) != null) {
                ...
                CompletableFuture<Void> future = CompletableFuture.runAsync(() -> {
                    try {
                        processAndWriteRecord(finalRecord, original, context);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                });
            ...
            CompletableFuture.allOf(futures.toArray(COMPLETABLE_FUTURES)).join();
            closeWriters();
    ...
    protected synchronized void writeRecord(RecordSchema outputSchema, Record outputRecord, Relationship relationship) throws SchemaNotFoundException, IOException {

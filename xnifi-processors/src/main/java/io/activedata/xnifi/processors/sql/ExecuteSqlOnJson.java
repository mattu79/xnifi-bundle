...
@CapabilityDescription("将JSON记录作为参数执行SQL语句")
public class ExecuteSqlOnJson extends AbstractJsonRowBatchProcessor {

    private static final BeanUtil BU = BeanUtil.silent;
    ...
    public static PropertyDescriptor PROP_SQL_MODE = new PropertyDescriptor.Builder()
            ...
            .displayName("SQL执行模式")
            .description("SQL执行模式，数据查询类：SELECT/PROCEDURE；数据操作类：INSERT/UPDATE/DELETE")
            .allowableValues(SQLMODE_QUERY, SQLMODE_UPDATE)
            .required(true)
    ...
    public static PropertyDescriptor PROP_SQL = new PropertyDescriptor.Builder()
            ...
            .description("待执行的SQL语句，可使用\":field\"来引用JSON记录的值。注意':'不能再引号中且前面必须有空格！")
            .required(true)
            .addValidator(StandardValidators.NON_BLANK_VALIDATOR)
            .build();
    ...
    protected Output handleProcessRow(Map<String, String> attributes, Input input, FlowFile flowFile, ProcessContext context) throws ProcessException {
        ...
        try {
            ...
            throw new BizException("执行SQL语句时出现错误：" + ExceptionUtils.getStackTrace(e));
        }
    }
    ...
    private Output excuteSqlWithMode(Map<String, String> attributes, Input input) throws SQLException {
       ...
       if (SQLMODE_UPDATE.equals(sqlMode)) {
            ...
            if (paramNames != null && paramNames.length > 0) {
                ...
                return createOutput(resultsList);
            } else {
     ...
     * @return
     */
    private static Output createOutput(List<List<Map<String, Object>>> resultsList) {
        Output output = new Output();
        if (resultsList != null) {

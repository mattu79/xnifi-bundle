...
import io.activedata.xnifi.neo4j.AbstractNeo4jCypherProcessor;
 ...
 * </ol>
 */
public class Neo4JCypherOnJsonCQLTests {
    ...
    public void setUp() throws Exception {
        runner = TestRunners.newTestRunner(Neo4jCypherOnJson.class);
        runner.setProperty(AbstractNeo4jCypherProcessor.CONNECTION_URL, neo4jUrl);
        runner.setProperty(AbstractNeo4jCypherProcessor.USERNAME, user);
        runner.setProperty(AbstractNeo4jCypherProcessor.PASSWORD, password);
        runner.setProperty(AbstractNeo4jCypherProcessor.QUERY, "match (n) return n");
    ...
    public void testCreateNodeNoReturn() throws Exception {
        runner.setProperty(AbstractNeo4jCypherProcessor.QUERY, "create (n)");
        ...
        assertEquals("1",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.NODES_CREATED));
    ...
    public void testCreateNodeOnePropertyWithReturn() throws Exception {
        runner.setProperty(Neo4jCypherOnJson.QUERY, "create (n { name:'abc' }) return n.name");
        ...
        assertEquals("0",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.RELATIONS_CREATED));
        assertEquals("0",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.RELATIONS_DELETED));
        assertEquals("1",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.PROPERTIES_SET));
        assertEquals("1",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.ROWS_RETURNED));
    ...
    public void testCreateNodeTwoPropertyOneLabelWithReturn() throws Exception {
        runner.setProperty(Neo4jCypherOnJson.QUERY, "create (n:Person { name:'abc', age : 1 }) return n.name, n.age");

        runner.enqueue(new byte[] {});
        runner.run(1,true,true);
        runner.assertAllFlowFilesTransferred(Neo4jCypherOnJson.REL_SUCCESS, 1);
        List<MockFlowFile> flowFiles = runner.getFlowFilesForRelationship(Neo4jCypherOnJson.REL_SUCCESS);
        assertEquals("1",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.LABELS_ADDED));
        assertEquals("1",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.NODES_CREATED));
        assertEquals("0",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.NODES_DELETED));
        assertEquals("0",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.RELATIONS_CREATED));
        assertEquals("0",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.RELATIONS_DELETED));
        assertEquals("2",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.PROPERTIES_SET));
        assertEquals("1",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.ROWS_RETURNED));
    ...
    public void testCreateTwoNodeTwoPropertyOneRelationshipWithReturn() throws Exception {
        ...
        runner.assertAllFlowFilesTransferred(Neo4jCypherOnJson.REL_SUCCESS, 1);
        List<MockFlowFile> flowFiles = runner.getFlowFilesForRelationship(Neo4jCypherOnJson.REL_SUCCESS);
        assertEquals("2",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.LABELS_ADDED));
        assertEquals("2",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.NODES_CREATED));
        assertEquals("0",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.NODES_DELETED));
        assertEquals("1",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.RELATIONS_CREATED));
        assertEquals("0",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.RELATIONS_DELETED));
        assertEquals("3",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.PROPERTIES_SET));
        assertEquals("1",flowFiles.get(0).getAttribute(Neo4jCypherOnJson.ROWS_RETURNED));
    ...
    public void testCreateNodeOnePropertyWithReturnWithEL() throws Exception {
        ...
        runner.setProperty(Neo4jCypherOnJson.QUERY, "${query}");
    ...
    public void testCreateTwoNodesWithReturn() throws Exception {
        runner.setProperty(Neo4jCypherOnJson.QUERY, "create (m:Person { name:'abc'}) create (n:Person { name : 'pqr'}) return n.name, m.name");

        runner.enqueue(new byte[] {});
        runner.run(1,false,true);
        runner.assertAllFlowFilesTransferred(Neo4jCypherOnJson.REL_SUCCESS, 1);

        runner.setProperty(Neo4jCypherOnJson.QUERY, "match (n) return n.name");
        runner.enqueue(new byte[] {});
        runner.run(1,true,true);
        ...
        assertEquals("0",flowFiles.get(1).getAttribute(Neo4jCypherOnJson.LABELS_ADDED));
        assertEquals("0",flowFiles.get(1).getAttribute(Neo4jCypherOnJson.NODES_CREATED));
        assertEquals("0",flowFiles.get(1).getAttribute(Neo4jCypherOnJson.NODES_DELETED));
        assertEquals("0",flowFiles.get(1).getAttribute(Neo4jCypherOnJson.RELATIONS_CREATED));
        assertEquals("0",flowFiles.get(1).getAttribute(Neo4jCypherOnJson.RELATIONS_DELETED));
        assertEquals("0",flowFiles.get(1).getAttribute(Neo4jCypherOnJson.PROPERTIES_SET));
        assertEquals("2",flowFiles.get(1).getAttribute(Neo4jCypherOnJson.ROWS_RETURNED));
